...
\begin{table}
	...
	\label{tab:prior}
	\begin{tabularx}{\columnwidth}{lcccr} % four columns, alignment for each
		\hline
		Parameter: $ \; \theta$ & & & & Prior: $ \; p(\theta)$ \\
		\hline
		$b_{g}^{(i)}$  & & & & $\mathcal{U}\Big(1,3.5\Big)$  \\
		$\Delta z^{(i)}$ & & & & $\mathcal{N}\Big(0, \delta^{2}_{z^{(i)}}\Big)$  \\
		\hline
	\end{tabularx}
\end{table}

...
\end{tabular}
\caption{\label{fig:star_galaxy_II}  Right Panel: At redshifts above $z_{\rm red}>0.4$ red-sequence galaxies (shown in red) and high confidence stars (shown in blue) reside in separated regions of the two-dimensional $(r-K_{\rm s}) \times (r-z)$ colors. Shown in pink dashed line is the predicted decision boundary between the two classes. The red-sequence candidates falling below the predicted boundary are marked by open circle. These objects are masked as likely stellar objects in the final catalog, and thus removed from our large-scale structure analysis. Left Panel: Purity fraction of the dense  (green dashed line) and the luminous (orange dashed line) samples as function of redshift.} 
\end{figure*}
